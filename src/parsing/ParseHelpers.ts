...
export class ParseHelpers {
  ...
  static parseContextToReferencedTable(ctx: any, input: string) {
    ...
    if (ctx._database !== undefined) {
      ...
      databaseName = databaseLocation.getToken(input);
    }
    ...
    if (ctx._schema !== undefined) {
      ...
      schemaName = schemaLocation.getToken(input);
    }
    ...
    if (ctx._table !== undefined) {
      ...
      tableNameOrAlias = tableLocation.getToken(input);
    } else {
